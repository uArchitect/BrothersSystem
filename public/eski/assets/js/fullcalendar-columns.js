    ...
    fc.views.multiColAgenda = AgendaView.extend({
        ...
        renderEvents: function(events) {
            ...
            for (var i = 0; i < events.length; i++) {
                ...
                // Etkinlik için sütun bilgisini doğru hesapla
                var fakeEvent = this._computeFakeEvent(event);
                fakeEvent.column = event.column || 0;  // Eğer `column` mevcutsa, onu kullan, yoksa 0 olarak ayarla
                this.fakeEvents.push(fakeEvent);
            }

            return AgendaView.prototype.renderEvents.call(this, this.fakeEvents);
        },
        ...
        trigger: function(name, thisObj) {
            ...
            if (name == 'eventRender' || name == 'eventAfterRender'
                ...
                // eventClick tetiklendiğinde sütun bilgisini al ve düzgün bir şekilde alert et
                if (name == 'eventClick') {
                    var event = args[2];
                    var columnIndex = event.column;  // Sütun bilgisini al
                    if (columnIndex !== undefined) {
                        alert('İşlem yapılan sütun: ' + this.columnHeaders[columnIndex]);
                    } else {
                        alert('Sütun bilgisi bulunamadı.');
                    }
                }
            }
            else if (
