...
var initApp = (function(app) {
	...
	app.listFilter = function (list, input, anchor) {
		...
		$(input).change( function () {
			...
			if(filter.length > 1) {
				...
				if (listPrev){	
					listPrev.text("Toplam " + $(list).find('[data-filter-tags]').length + " öğeden " + $(list).find('li.js-filter-show').length + " tanesi gösteriliyor");				
				}
	...
	app.factoryReset = function () {
		...
		if (typeof bootbox  != 'undefined') {
			...
			if (confirm( 'Tüm ayarlarınızı sıfırlamak üzeresiniz. Bu işlem geri alınamaz. Tüm düzen ayarlarınızı kaybedeceksiniz. Onaylıyor musunuz?')) {
				localStorage.clear();
				initApp.resetSettings();
				location.reload();
			}
				...
				switch ( true ) {
					...
					case ( actiontype === 'panel-close' ):
						...
						if (typeof bootbox  != 'undefined') {
							...
							bootbox.confirm({
								title: "<i class='" + myapp_config.appIconPrefix +  " fa-times-circle text-danger mr-2'></i> Paneli silmek istediğinizden emin misiniz? <span class='fw-500'>&nbsp;'" + selectedPanel.children('.panel-hdr').children('h2').text().trim() + "'&nbsp;</span>?",
								message: "<span><strong>Uyarı:</strong> Bu işlem geri alınamaz!</span>",
								centerVertical: true,
								swapButtonOrder: true,
								buttons: {
									confirm: {
